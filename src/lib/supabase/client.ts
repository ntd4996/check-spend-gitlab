...
import type { TimeSpentByMonth, TimeSpentByYear } from "@/types";
import {
  client,
  CURRENT_USER_QUERY,
  TIME_SPENT_QUERY,
  ...
  try {
    ...
    while (hasNextPage) {
      ...
      const timeSpentData = projects.nodes.flatMap((project: GitLabProject) => {
        ...
        return project.issues.nodes.flatMap((issue: GitLabIssue) => {
          ...
          return issue.timelogs.nodes
            .filter(
              (timelog: GitLabTimelog) =>
                timelog?.user?.username === currentUser.username
            )
      ...
      onProgress(
        ...
        `Đã tìm thấy ${allTimeSpentData.length} bản ghi...`
      );
    }
...
export const getTimeSpentByMonth = async (
  month: string
): Promise<TimeSpentByMonth[]> => {
  const { data, error } = await supabase.rpc("get_time_spent_by_month", {
...
export const getTimeSpentByYear = async (
  year: string
): Promise<TimeSpentByYear[]> => {
  const { data, error } = await supabase.rpc("get_time_spent_by_year", {
    p_user_email: userEmail,
    p_year: year,
  });
